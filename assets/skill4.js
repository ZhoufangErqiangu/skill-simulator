...
function Disable(parts) {
  ...
  if (cssnum == "1") {
    ...
    document.getElementById(parts).style.color = "#666666";
    document.getElementById(parts).style.border = "dashed 1px";
  }
}
...
function Visible(parts) {
  if (cssnum == "1") {
    ...
    document.getElementById(parts).style.backgroundColor = "#FFFFEE";
    ...
    document.getElementById(parts).style.backgroundColor = "#FFFFEE";
    document.getElementById(parts).style.color = "#000000";
    document.getElementById(parts).style.border = "outset 1px";
  }
}
...
function showMsg(type) {
  if (document.Msg.jobchg1)
    jobchange1 = eval(
      document.Msg.jobchg1.options[document.Msg.jobchg1.selectedIndex].value,
    );
  if (document.Msg.jobchg2)
    jobchange2 = eval(
      document.Msg.jobchg2.options[document.Msg.jobchg2.selectedIndex].value,
    );
  var disp1 =
    (disp2 =
    disp3 =
  ...
  if (type < 2) {
    ...
    // ポイント未処理時
    if (use1 + 1 < jobchange1)
      disp1 = "<span style='color:#aa0033;'>" + disp1 + "</span>";
    if (use2 + 1 < maxjob - subp1)
      disp2 = "<span style='color:#aa0033;'>" + disp2 + "</span>";
    ...
    if (use1 + use2 + 2 > jobchange1 + maxjob || use2 + 1 > maxjob) {
      ...
      document.getElementById("msgid").innerHTML =
        ...
        maxjob +
        "　over : " +
        eval(use1 + use2 + 2 + (rest1 + rest2) - (jobchange1 + maxjob));
    }
  }
  if (type == 2) {
    if (use1 <= jobchange1 - 1) {
    ...
    if (use1 + 1 < jobchange1)
      disp1 = "<span style='color:#aa0033;'>" + disp1 + "</span>";
    if (use2 + 1 < jobchange2 - subp1)
      disp2 = "<span style='color:#aa0033;'>" + disp2 + "</span>";
    if (use3 + 1 < 50 - subp2 - subp3)
      disp3 = "<span style='color:#aa0033;'>" + disp3 + "</span>";
...
function Point(slv, mlv, skill) {
  ...
  document.getElementById(skill).style.backgroundPosition =
    leftPos + "px -" + slv * 28 + "px";
}
...
function Init() {
  ...
  if (getCookie("css") > 0) chgStyle(getCookie("css"));
  ...
  tmp = getCookie("ro4");
  for (i = 0; i < 32; i++) slotdata[i] = tmp.split("|")[i];

  // スロットセレクト
  latestSlotNum = 0;
  for (i = 30; i > 0; i--) {
    if (slotdata[i].split(":")[0] == "") latestSlotNum = i - 1;
  }
  for (i = 30; i > 0; i--) {
  ...
  for (i = 1; i <= document.Msg.Data.options.length; i++) {
    if (slotdata[i] != ":::") {
      document.Msg.Data.options[i - 1].text =
        ...
        i +
        " : (" +
        unescape(slotdata[i].split(":")[1]) +
        ") " +
        unescape(slotdata[i].split(":")[3]);
    }
    if (slotdata[i] == ":::") {
      document.Msg.Data.options[i - 1].text = "slot " + i + " : no save";
    }
  }
}
...
function Save2(code) {
  ...
  if (slotclass != job && tmp.indexOf(" : no save") == -1) {
    setAlertLayer(
      "<br><br><br><br>指定档案夹有其他职业资料。<br>确定要覆盖掉吗？<br><br><br>" +
        "<form><input type='button' value='　OK　' onclick='Save3(code);'>　　" +
        "<input type='button' value='CANSEL' onclick='offOptionLayer();'></form>",
      0,
    );
  } else {
    Save3(code);
...
function Save3(code) {
  ...
  if (slotname == "no Save Data") slotname = "no Name";
  ...
  document.Msg.Data.options[document.Msg.Data.selectedIndex].text =
    "slot " + num + " : (" + job + ") " + slotname;
  document.Msg.Data.options[
    document.Msg.Data.selectedIndex
  ...
  setAlertLayer("Slot" + num + " 储存完毕。", 1);
}

function codeCopy() {
  tmp1 = "http://fual.jakou.com/skills/" + job + ".html?" + Save("scode");
  tmp2 =
    "http://fual.jakou.com/index.htm?skills/" + job + ".html?" + Save("scode");
  setAlertLayer(
    "<br><br>URL输出如下。<br>（非IE浏览器的使用者，请自行复制。）<br><br><br>" +
      "无页框技能树:" +
      "<form><input type='text' value='" +
      tmp1 +
      "' onfocus='this.select();' onclick='this.select();' size=60 readonly></form><br>" +
      "有页框技能树:" +
      "<form><input type='text' value='" +
      tmp2 +
      "' onfocus='this.select();' onclick='this.select();' size=60 readonly></form>",
    0,
  );
}
...
function decode(code, ver) {
  ...
  if (code.substring(0, 2) != ver) {
    setAlertLayer("不符合的编码，或是不同版本的编码。请使用相同版本的编码。");
  ...
  for (i = 0; i < tmp.length / 2; i++) {
    var x = (y = 0);
    x = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(
      tmp.charAt(2 * i),
    );
    y = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(
      tmp.charAt(2 * i + 1),
    );
    tmp2 = 52 * x + y;
...
function loadType(mode) {
  ...
  if (mode == "cookie") {
    if (
      slotdata[num].split(":")[0] != job &&
      slotdata[num].split(":")[0] != ""
    ) {
      tmp = num + "|";
      for (i = 1; i < 31; i++) {
        tmp += slotdata[i] + "|";
      }
      document.cookie = "sq2=" + tmp + "; expires=Fri, 31-Dec-2030 23:59:59; ";
...
function Reset() {
  setAlertLayer(
    "<br><br><br><br>全部技能点已重置。<br><br><br><br>" +
...
function DeleteSlot2() {
  ...
  setAlertLayer("資料已刪除：(slot " + num + ")", 1);
}
...
function msovr(posX, posY, skillid, flg) {
  ...
  if (SkillP > 0)
    tmpTxt =
      tmpTxt.substring(0, tmpTxt.lastIndexOf("<hr>") + SkillP * 4 + 5) +
      "<span style='color:#ff0066;'>" +
      tmpTxt.substring(
        tmpTxt.lastIndexOf("<hr>") + SkillP * 4 + 5,
        tmpTxt.lastIndexOf("<hr>") + SkillP * 4 + 8,
      ) +
      "</span>" +
  ...
  document.getElementById("skill").innerHTML =
    "<div id='skillbox' style='padding:10px; line-height:14px;'>" +
    tmpTxt +
    "</div>";
...
function setAlertLayer(txt, close) {
  ...
  if (typeof document.body.style.maxHeight != "undefined") {
    document.getElementById("option1").style.backgroundImage =
      "url(img/alpha.png)";
  } else {
    document.getElementById("option1").style.backgroundImage =
      "url(img/black.png)";
  ...
  tmpMsg +=
    "><tr><td align=center><div class='alert'>\n" +
    txt +
    "</div></td></tr></table>";
